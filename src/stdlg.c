...
static DWORD WINAPI CheckForFidoThread(LPVOID param)
{
	...
	// Use Fido-RU-BY fork URL (no server check, no reachability check)
	fido_url = safe_strdup(FIDO_RU_BY_URL);
	// Always enable the ISO download split button
...
{
	// RU/BY: Always enable Fido (no PowerShell or update check requirement)
...
INT_PTR CALLBACK NewVersionCallback(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
		...
		SetWindowTextU(GetDlgItem(hDlg, IDC_DOWNLOAD_URL), update.download_url != NULL ? update.download_url : "");
		SendMessage(GetDlgItem(hDlg, IDC_PROGRESS), PBM_SETRANGE, 0, (MAX_PROGRESS<<16) & 0xFFFF0000);
		// Download button always enabled (no checks)
		ResizeButtonHeight(hDlg, IDCANCEL);
		SetDarkModeForChild(hDlg);
	...
	case WM_COMMAND:
		...
		case IDC_DOWNLOAD:	// Also doubles as abort and launch function
			...
			case 2:		// Launch newer version and close this one
				...
				// Signature check disabled for RU/BY - always allow launch
	...
	case UM_PROGRESS_EXIT:
		...
		if (wParam != 0) {
			...
			// Download button always stays enabled (no signature check)
			download_status = 0;
		}
		return (INT_PTR)TRUE;
	}
