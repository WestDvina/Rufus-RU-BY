...
//#define FORCE_URL "https://cdimage.debian.org/debian-cd/current/amd64/iso-cd/debian-9.8.0-amd64-netinst.iso"
	...
	if (fido_script == NULL) {
		...
		// Этот блок был закомментирован, так как наш кастомный скрипт Fido-RU-BY
		// не подписан оригинальным ключом разработчика Rufus.
...
#ifndef RUFUS_TEST
	/*
	// *** Отключение Authenticode подписи ***
	// Этот блок проверяет цифровую подпись самого .ps1 файла.
	// Так как наш скрипт не подписан, мы отключаем эту проверку.
