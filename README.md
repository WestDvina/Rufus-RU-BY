...
Этот репозиторий содержит форк (модифицированную версию) официальной утилиты [Rufus](https://rufus.ie/), в который интегрирован кастомный скрипт **IWID (ISO Windows Interactive Downloader)**.

## Особенности сборки Rufus@WD

Данная сборка Rufus (с меткой `@WD` в заголовке) обеспечивает:

1.  **Обход геоблокировок и исправленная загрузка IWID:** В код Rufus жёстко прописана ссылка на [IWID](https://github.com/WestDvina/ISO_win_links).
2.  **Отключенная проверка подписи (Критично):** Отключены обе проверки цифровой подписи (файла `.lzma` и Authenticode скрипта `.ps1`), что позволяет запускать нашу неподписанную версию IWID.
...
4.  **Актуальность:** Кодовая база синхронизирована с последней версией Rufus.

---

## Новые релизы Rufus RU/BY — Стабильный доступ к образам Windows

В техническом обновлении v.4.12.1 мы кардинально пересмотрели механизм загрузки ISO-образов, чтобы сделать его надёжнее для пользователей из России и Беларуси.

**Что изменилось:**

Замена Fido на IWID: Мы отказались от нашего форка скрипта Fido в пользу IWID (ISO Windows Interactive Downloader). Это позволило обойти частые ошибки «скрипт не найден» или «загрузка недоступна».

PowerShell-оптимизация: Новый загрузчик работает напрямую через PowerShell, обеспечивая более стабильное соединение с серверами Microsoft.

Умный поиск: Поиск оригинальных ISO стал быстрее и точнее.

Это обновление рекомендуется всем, у кого возникали трудности с кнопкой «Скачать» в оригинальном Rufus
...
### ❓ Почему в Rufus не появляется кнопка «Скачать»?
